...
try {
    ...
    generatePassword = (exported && exported.generatePassword) || sandbox.generatePassword || (sandbox.window && sandbox.window.generatePassword);
    maskPassword = (exported && exported.maskPassword) || sandbox.maskPassword || (sandbox.window && sandbox.window.maskPassword);
    formatUserName = (exported && exported.formatUserName) || sandbox.formatUserName || (sandbox.window && sandbox.window.formatUserName);
} catch (e) {
    generatePassword = sandbox.generatePassword || (sandbox.window && sandbox.window.generatePassword);
...
if (!generatePassword || !maskPassword || !formatUserName) throw new Error('system-utils の関数が取得できませんでした');
...
try {
    // 生成長さと基本的な条件
    const pw = generatePassword({ length: 16, useLower: true, useUpper: true, useNumbers: true, useSymbols: true });
    assert.strictEqual(typeof pw, 'string');
    assert.strictEqual(pw.length, 16, 'generatePassword length');

    // 禁止文字が含まれていないことを複数回生成して確認
    for (let i = 0; i < 20; i++) {
        const p = generatePassword({ length: 24, useLower: true, useUpper: true, useNumbers: true, useSymbols: true });
...
try {
    ...
    const p = generatePassword({ length: 12, useLower: true, useUpper: true, useNumbers: true, useSymbols: true });
