...
const defaultStubBank = (q, cb) => cb({ bankCode: q || '9999', bankName: 'テスト銀行', bankKana: 'ﾃｽﾄｷﾞﾝｺｳ' });
const defaultStubBranch = (bankCode, q, cb) => cb({ branchCode: q || '000', branchName: '本店', branchKana: 'ﾎﾝﾃﾝ' });
...
try {
  let threw = false;
  try { BANK.normalizeAccountNumber('12345678', 7); } catch (e) { threw = true; }
...
try {
  let threw = false;
  try { BANK.normalizePayeeName('yamada'); } catch (e) { threw = true; }
...
try {
  let threw = false;
  try { BANK.normalizePayeeName('山田 太郎'); } catch (e) { threw = true; }
...
try {
  let threw = false;
  try { BANK.normalizeEdiInfo('A,B'); } catch (e) { threw = true; }
...
try {
  let threw = false;
  try { BANK.normalizeEdiInfo('山田'); } catch (e) { threw = true; }
...
try {
  const stubBank = (code, cb) => cb({ bankCode: code, bankName: 'テスト銀行', bankKana: 'ﾃｽﾄｷﾞﾝｺｳ' });
  const stubBranch = (bankCode, branch, cb) => cb({ branchCode: branch, branchName: '本店', branchKana: 'ﾎﾝﾃﾝ' });
