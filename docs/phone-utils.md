...
電話番号の出力は、総務省2026年2月版の電気通信番号指定状況（電気通信番号計画（令和元年総務省告示第6号）第1第4項による公表）によります。

2026年3月1日版までの総務省公開情報を確認した結果、このライブラリの更新を要する差分はありませんでした。そのため、現行の電話番号設定を継続利用します。
...
現状メモ: 2026年3月1日版の公開情報までは確認済みで、今回の確認では `src/phone-utils.js` のデータ定義変更は不要でした。
