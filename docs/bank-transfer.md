...
```text
入力:  "株式会社 テストソリューション"
出力:  "ﾃｽﾄｿﾘｭｰｼｮﾝ ﾃﾞｨ"  // 例: "株式会社" -> 'ｶ' の短縮表現（実際の短縮は設定に依存）
```
...
```text
入力:  "東京営業所 山田太郎"
出力:  "ﾄｳｷﾖｳ ｴｲ 山田-ﾀﾛｳ"  // "営業所" -> 'ｴｲ' などに置換
```
...
```text
入力:  "山田―太郎株式会社"
出力:  "ﾔﾏﾀﾞ-ﾀﾛｳ ﾃﾞｨ"  // 長音が '-' に、名義が半角カナ化され、法人略語が縮約される
```

注: 実際の出力は `options`（略語適用のオン/オフ、切り詰めバイト数など）や内部のマップ定義に依存します。
