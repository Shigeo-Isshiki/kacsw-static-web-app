...
注意: 実装上 `getBank` は API レスポンスの `kana` フィールド（`j.kana`）を優先して使用します。`normalize.kana` は API の `kana` が存在しない場合のフォールバックとして扱われます。つまり、API が提供する生の `kana` をまず採用し、なければ `normalize.kana` を利用する挙動です。これは API 側がより正確な仮名表記を返す想定のためで、`normalize.kana` は補助的な値として扱われます。
