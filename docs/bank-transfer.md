...
- 2026-09-22: `generateZenginData`/`generateZenginDataAsync` で `headerData.typeCode` が `'11'`/`'12'`（給与振込・賞与振込）でも常に総合振込様式（EDI情報＋`7Y`）になってしまうバグを修正しました（`transferType` の二重解決が原因）。修正後、実際の金融機関システムへのアップロードで給与振込データとして正常に処理できることを確認済みです。
...
- `records` (Array<object>) — データ区分 `2` の明細レコードだけを抽出した配列です。ヘッダ・トレーラ・エンドは含みません。
  ...
  - `processResultCode` (string) — 処理結果コード。振込先の金融機関が全銀データの114バイト目に付加する処理結果コードをそのまま返します。
  - `processResultLabel` (string) — `processResultCode` を次の対応表で人が読める文言に変換したラベルです。表にない値は `"その他"` として扱います。

    | 処理結果コード | `processResultLabel` |
    | -------------- | -------------------- |
    | `"0"`          | 正常（処理済）       |
    | `"1"`          | 該当口座なし         |
    | `"2"`          | 氏名相違             |
...
- `normalizeEdiInfo(input, options)`
  ...
  - 挙動:
    ...
    - 半角化可能な英数記号（例: `（Ａ）` → `(A)`）は内部で半角化してから許容判定します。
    - 許容外文字が含まれている場合はエラーを投げます（エラーメッセージには入力時の元文字を表示）。
...
- `normalizePayeeName(name)`
  - 受取人名を全銀フォーマット向けに正規化するヘルパです。実装に沿った詳細な処理順は次の通りです。
    ...
    - 半角化可能な文字は先に半角化してから判定する。
    - 満たす場合は Shift_JIS 相当で先頭 30 バイトに切り詰めて返す（pad は行わない）。
    - 満たさない場合は、入力時の元文字を列挙して Error を投げる。
  - 引数:
    - `name` (string) — 元の受取人名
...
`normalizePayeeName` が最終的に許容する半角の記号は以下の通りです（実装に基づく）。

- スペース（U+0020）
- 丸括弧（`(` / `)`、U+0028 / U+0029）
- コンマ（`,`、U+002C） — 実装では全角カンマ（`，`）および日本語読点（`、`）を自動で半角コンマに正規化します。
- ハイフン（`-`、U+002D）
- ピリオド（`.`、U+002E）
- スラッシュ（`/`、U+002F）

また、半角カタカナ（U+FF61..U+FF9F）、英大文字（A–Z）、数字（0–9）も許容されます。全角の英数記号は半角へ正規化して判定されます。逆に次の ASCII 文字は明示的に許容されていません: アポストロフィ（`'`）、プラス（`+`）、コロン（`:`）、疑問符（`?`）、バックスラッシュ（`\`）など。

注意: EDI 用の補助情報を正規化する `normalizeEdiInfo` はコンマ（`,` および `，`）を許容しない設計です。EDI フィールドにコンマを含めるとエラーになる点にご注意ください。
