...
- `Error` を投げます。以下のケースが該当します:
  - 不正な形式（解析できない文字列など）
  - 無効な月（1～12 の範囲外）
  - 無効な日（1～31 の範囲外、または各月の最大日数を超える。例: 2月30日）
...
- 西暦形式（`YYYY-MM-DD`）での入力では、月が 1～12、日が各月の有効範囲内であることが厳密に検証されます。例えば `2026-13-27`（13月）や `2026-02-30`（2月30日）のような不在の日付はエラーになります。
