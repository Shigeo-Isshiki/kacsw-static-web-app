...
(function () {
   ...
   * 半角カタカナ変換用のマップを作成する
   */
  const _ch_halfWidthKanaMap = new Map(
    Object.entries(_ch_convert_character_list.half_width_kana),
  );
  /**
   * 全角カタカナ変換用のマップを作成する
   */
  const _ch_full_width_kana_map = new Map(
    Object.entries(_ch_convert_character_list.full_width_kana),
  );
  /**
   * 濁点・半濁点変換用のマップを作成する
   */
  const _ch_turbidity_kana_map = new Map(
    Object.entries(_ch_convert_character_list.turbidity_kana),
  );
  /**
  ...
  const _ch_replace_with_map = (str, pattern, map) => {
    if (!_ch_checkString(str))
      throw new Error(
        `_ch_replace_with_mapの[${str}]は文字である必要があります`,
      );
  ...
  const convert_to_half_width_kana = (str = "") => {
    ...
    const half_width_kana_pattern = _ch_buildPattern(
      _ch_halfWidthKanaMap.keys(),
    );
    return _ch_replace_with_map(
      str,
      half_width_kana_pattern,
      _ch_halfWidthKanaMap,
    );
  };
  ...
  const convert_to_full_width_kana = (str = "", hiragana_sw = true) => {
    ...
    const full_width_kana_pattern = _ch_buildPattern(
      _ch_full_width_kana_map.keys(),
    );
    const turbidity_kana_pattern = _ch_buildPattern(
      _ch_turbidity_kana_map.keys(),
    );
    let result = hiragana_sw ? hiraganaToKatakana(str) : str;
    result = _ch_replace_with_map(
      result,
      full_width_kana_pattern,
      _ch_full_width_kana_map,
    );
    result = _ch_replace_with_map(
      result,
      turbidity_kana_pattern,
      _ch_turbidity_kana_map,
    );
    return result;
  };
  ...
  const convert_to_email_address = (email_address = "") => {
    ...
    return email_pattern.test(single_byte_characters)
  ...
  const toHalfWidthKana = (str = "", throwOnError = true) => {
    ...
    if (errorChar)
      throw new Error(`変換不能な文字が含まれています: ${errorChar}`);
    return result;
  };
  ...
  const assertEmailAddress = (emailAddress = "") => {
    ...
    try {
      ...
      if (/\.\.|^\.|\.@|@\.|\.$/.test(singleByteCharacters))
        throw new Error(
          "メールアドレスは連続ドットや@直前・直後のドットを含めることはできません",
        );
  ...
  if (typeof window !== "undefined") {
    ...
    window.convert_to_double_byte_characters =
