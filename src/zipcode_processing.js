...
const _zp_getZipcodeOnly = (str) => {
	...
	const toHalfWidthAlphaNum = (s) =>
		s.replace(_zp_zenkakuAlphaNumReg, (c) => String.fromCharCode(c.charCodeAt(0) - 0xfee0));
	// 半角化→大文字化→記号除去
...
const _zp_buildAddressString = (addr) => {
	return [addr.pref_name, addr.city_name, addr.town_name, addr.block_name ? addr.block_name : '']
		.filter(Boolean)
		.join('');
};
// --- APIレスポンスバリデーション ---
