...
const _zp_getZipcodeOnly = (str) => {
  if (!str) {
    throw new _zp_ZipcodeProcessingError(
      "郵便番号は文字列である必要があります",
      _zp_ERROR_TYPE.LOGIC,
    );
  }
  // 全角英数字→半角英数字
  const toHalfWidthAlphaNum = (s) =>
    s.replace(_zp_zenkakuAlphaNumReg, (c) =>
      String.fromCharCode(c.charCodeAt(0) - 0xfee0),
    );
  // 半角化→大文字化→記号除去
  const result = toHalfWidthAlphaNum(String(str))
    .toUpperCase()
    .replace(_zp_removeSymbolsReg, "");
  if (!result || !/^\d+$|^[A-Z0-9]+$/.test(result)) {
    throw new _zp_ZipcodeProcessingError(
      `郵便番号として無効です（数字・大文字英字以外の文字が含まれているか、数字・英字が含まれていません）: ${str}`,
      _zp_ERROR_TYPE.LOGIC,
    );
  }
  return result;
...
const zipcodeFormatting = (zipcode) => {
  ...
  const zip_code_right = zipcodeNO.substring(
    _zp_ZIPCODE_LEFT,
    _zp_ZIPCODE_LENGTH,
  );
...
const getZipcodeAddress = (zipcode, successCallback, failureCallback) => {
  if (typeof successCallback !== "function")
    throw new _zp_ZipcodeProcessingError(
      "住所取得後に呼び出されるコールバックは関数である必要があります",
      _zp_ERROR_TYPE.LOGIC,
    );
  if (failureCallback !== undefined && typeof failureCallback !== "function") {
    throw new _zp_ZipcodeProcessingError(
      "住所取得失敗時に呼び出されるコールバックは関数または未指定である必要があります",
      _zp_ERROR_TYPE.LOGIC,
    );
  }
  const zipcodeNO = _zp_getZipcodeOnly(zipcode);
  if (
    zipcodeNO.length !== _zp_ZIPCODE_LEFT &&
    zipcodeNO.length !== _zp_ZIPCODE_LENGTH
  ) {
    throw new _zp_ZipcodeProcessingError(
      "郵便番号は3桁または7桁である必要があります",
      _zp_ERROR_TYPE.LOGIC,
    );
  }
    ...
    .then((response) => {
      if (!response.ok) {
        if (response.status === 404) {
          throw new _zp_ZipcodeProcessingError(
            `郵便番号／デジタルアドレス「${zipcodeNO}」に該当する住所が見つかりません`,
            _zp_ERROR_TYPE.AJAX,
          );
      ...
      if (result.addresses.length === 1) {
        ...
        successCallback({
          ...
          city: addr.city_name
            ...
            : null,
          town: addr.town_name
            ? addr.town_name.replace(/[\u3000\u0020]/g, "")
            : null,
          block: addr.block_name
            ? addr.block_name.replace(/[\u3000\u0020]/g, "")
            : null,
          other_address: addr.other_name
            ? addr.other_name.replace(/[\u3000\u0020]/g, "")
            : null,
          office: addr.biz_name
            ? addr.biz_name.replace(/[\u3000\u0020]/g, "")
            : null, // APIのbiz_nameはofficeとして返却する
        ...
        if (failureCallback) {
          failureCallback(
            new _zp_ZipcodeProcessingError(
              `郵便番号／デジタルアドレス「${zipcode}」に該当する住所が複数見つかりました。郵便番号／デジタルアドレスを確認して再検索してください。`,
              _zp_ERROR_TYPE.AJAX,
            ),
          );
        }
    ...
    .catch((err) => {
      if (failureCallback) {
        if (!(err instanceof _zp_ZipcodeProcessingError)) {
          err = new _zp_ZipcodeProcessingError(
            err && err.message
              ? err.message
              : "ネットワークエラーまたは不明なエラー",
            _zp_ERROR_TYPE.AJAX,
...
if (typeof window !== "undefined") {
  window.getZipcodeAddress = getZipcodeAddress;
}
