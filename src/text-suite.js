...
/* exported isSingleByteAlnumOnly, toFullWidthKatakana, toFullWidth, toFullWidthHiragana, toHalfWidthKana, toHalfWidth, assertEmailAddress */
"use strict";
...
const _TS_HALF_WIDTH_KANA_MAP = new Map(
  Object.entries(_TS_CONVERT_CHARACTER_LIST.halfWidthKana),
);
// 半角カタカナから全角カタカナへの変換テーブルから生成するマップ（各種変換処理で利用）
const _TS_FULL_WIDTH_KANA_MAP = new Map(
  Object.entries(_TS_CONVERT_CHARACTER_LIST.fullWidthKana),
);
// 濁点・半濁点の変換テーブルから生成するマップ（各種変換処理で利用）
const _TS_TURBIDITY_KANA_MAP = new Map(
  Object.entries(_TS_CONVERT_CHARACTER_LIST.turbidityKana),
);
...
// これらは現在リポジトリ内で未使用のため、外部露出は行いません。
...
const toFullWidthKatakana = (str = "", throwOnError = true) => {
  ...
  // 半角カタカナ→全角カタカナ
  work = work.replace(
    fullWidthKanaPattern,
    (char) => _TS_FULL_WIDTH_KANA_MAP.get(char) ?? char,
  );
  // 合成濁点・半濁点（カ゛→ガ等）を変換
  work = work.replace(
    turbidityKanaPattern,
    (pair) => _TS_TURBIDITY_KANA_MAP.get(pair) ?? pair,
  );
  ...
  if (errorChar)
    throw new Error(`全角カタカナ以外の文字が含まれています: ${errorChar}`);
  return work;
};
...
const toFullWidth = (str = "", throwOnError = true) => {
  ...
  try {
    ...
    const fullWidthStr = [...replaced]
      ...
      .join("");
    if (errorChar)
      throw new Error(`全角文字に変換不能な文字が含まれています: ${errorChar}`);
    return fullWidthStr;
  } catch (error) {
    throw error;
  }
...
const toFullWidthHiragana = (str = "", throwOnError = true) => {
  ...
  if (errorChar)
    throw new Error(`ひらがな以外の文字が含まれています: ${errorChar}`);
  return work;
};
...
const toHalfWidthKana = (str = "", throwOnError = true) => {
  ...
  if (errorChar)
    throw new Error(`半角カタカナ以外の文字が含まれています: ${errorChar}`);
  return result;
};
...
const toHalfWidth = (str = "", throwOnError = true) => {
  ...
  try {
    ...
    const halfWidthStr = [...halfWidthKana]
      ...
      .join("");
    if (errorChar)
      throw new Error(`半角文字に変換不能な文字が含まれています: ${errorChar}`);
    return halfWidthStr;
  } catch (error) {
    throw error;
  }
...
const assertEmailAddress = (emailAddress = "") => {
  ...
  if (!_ts_checkString(emailAddress))
