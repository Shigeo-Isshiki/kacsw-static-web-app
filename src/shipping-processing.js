...
const _SP_SHIPPING_INQUIRY_URL_MAP = {
  yamato:
    "https://member.kms.kuronekoyamato.co.jp/parcel/detail?pno={trackingNumber}", // ヤマト運輸
  japanpost:
...
const getNextBusinessDay = (
  ...
  if (
    !Number.isInteger(cutoffHourNum) ||
    cutoffHourNum < 0 ||
    cutoffHourNum > 23
  ) {
...
const validateTrackingNumber = (
  ...
  if (!_sp_checkString(trackingNumber))
    throw new Error("伝票番号は文字列である必要があります");
  ...
  processed = processed.replace(/[０-９]/g, (s) =>
  ...
  if (!/^[0-9\- ]+$/.test(processed)) {
    ...
    throw new Error(
      `伝票番号には半角数字・ハイフン・空白のみ許容されます。不正な文字: ${invalid}`,
    );
  }
  // ハイフンと空白を除去して数字のみにする
