...
/**
 * 銀行送信用にわかりやすくした営業日取得関数（コールバック形式）
 * options:
 *  - holidays: Array of 'YYYY-MM-DD' strings を渡すと API を使わずそれで祝日判定を行う
 *  - holidayApiBaseUrl: 祝日APIのベースURLを上書きする（未指定時は内部定数を使用）
 * シグネチャ: nextBankBusinessDay(baseDate = new Date(), cutoffHour = 16, options = {}, callback)
 */
const nextBankBusinessDay = (baseDate = new Date(), cutoffHour = 16, options = {}, callback) => {
	if (typeof callback !== 'function') {
		throw new Error('callback は関数である必要があります');
	}
	// options 省略対応: third arg を callback として呼ばれた場合をサポート
	...
	const holidays = Array.isArray(options.holidays) ? options.holidays : null;
	const holidayApiBase = options.holidayApiBaseUrl || _SP_HOLIDAY_API_BASE_URL;

	// 内部: 与えられた日付が祝日かどうか（callback形式）。holidays があれば同期判定する。
	const _isHoliday = (date, cb) => {
		if (holidays) {
			const y = date.getFullYear();
			const m = String(date.getMonth() + 1).padStart(2, '0');
			const d = String(date.getDate()).padStart(2, '0');
			const key = `${y}-${m}-${d}`;
			cb(holidays.indexOf(key) !== -1);
			return;
		}
		// フェールバック: 既存の API 呼び出しを利用
		const year = date.getFullYear();
		const month = String(date.getMonth() + 1).padStart(2, '0');
		const day = String(date.getDate()).padStart(2, '0');
		const dateStr = `${year}-${month}-${day}`;
		fetch(`${holidayApiBase}/${dateStr}`)
			.then((response) => {
				if (!response.ok) {
					cb(false);
					return;
	...
	const _isBusinessDay = (date, cb) => {
		...
		_isHoliday(date, (isHoliday) => {
			cb(!isHoliday);
		});
	};

	const findBusinessDay = () => {
		_isBusinessDay(targetDate, (isBusinessDay) => {
			if (isBusinessDay) {
				const year = targetDate.getFullYear();
				const month = String(targetDate.getMonth() + 1).padStart(2, '0');
				const day = String(targetDate.getDate()).padStart(2, '0');
