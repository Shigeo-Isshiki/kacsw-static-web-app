...
const _pu_isDigit11PhoneNumberRange = (code) => {
  const allPrefixes = Object.values(
    _PU_PHONE_NUMBER_CONFIG.digit11PhoneNumberRange,
  ).flat();
...
const _pu_getPhoneNumberOnly = (str) => {
  ...
  const result = String(str)
    ...
    .replace(_PU_ZENKAKU_NUM_REGEX, (s) =>
...
const _pu_getPhoneType = (number) => {
  ...
  if (num.length === 11) {
    ...
    if (_pu_getDigit11PrefixList("service").includes(prefix4))
      return "着信課金";
    if (_pu_getDigit11PrefixList("wireless").includes(prefix4))
      return "無線呼出";
  ...
  if (
    num.length === 10 &&
    _pu_isNotLandlinePhoneNumberRange(num.substring(0, 4))
  ) {
    const prefix4 = num.substring(0, 4);
...
const _pu_isValidJapanesePhoneNumber = (str) => {
  ...
  for (let c = 0, l = areaCodeList.length; c < l; c++) {
    ...
    if (localLen) {
      ...
      if (
        num.length ===
        areaCodeLen + localLen + (num.length - (areaCodeLen + localLen))
      ) {
        ...
        if (
          ...
          return true;
        if ((areaCode === "0570" || areaCode === "0990") && num.length === 10)
          return true;
      }
    }
  }
...
const isValidPhoneNumber = (phoneNumber) => {
  if (!phoneNumber) {
    throw new Error(
      "電話番号が指定されていません。文字列または数値を入力してください。",
    );
  }
...
if (typeof window !== "undefined") {
  ...
  try {
    window.getPhoneNumberType =
      typeof getPhoneNumberType !== "undefined"
        ? getPhoneNumberType
        : undefined;
  } catch {}
  try {
    window.normalizePhoneNumber =
      typeof normalizePhoneNumber !== "undefined"
