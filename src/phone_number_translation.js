...
const _pn_phoneNumberData = {
  ...
  areaCodeRanges: {
    ...
    "0250": "20-99",
    "025":
      "200-216,218-399,500-516,518-569,590,592-609,700-716,718-719,750-790,792-809",
    "0254": "20-90,92-99",
    "0255": "70-89",
    "0256": "20-99",
    "0257": "20-49",
    ...
    "0820": "20-89",
    "082":
      "200-299,400-409,420-439,490-599,800-909,921-929,941-943,960-969,990-999",
    "0823": "20-99",
    "0824": "40-89",
    "0826": "20-89",
    "0827": "20-50,52-99",
...
const _pn_isDigit11PhoneNumberRange = (code) => {
  const allPrefixes = Object.values(
    _pn_phoneNumberData.digit11PhoneNumberRange,
  ).flat();
...
const _pn_getPhoneType = (number) => {
  ...
  if (num.length === 11) {
    ...
    if (_pn_getDigit11PrefixList("wireless").includes(prefix4))
      return "wireless";
  ...
  if (
    num.length === 10 &&
    _pn_isNotLandlinePhoneNumberRange(num.substring(0, 4))
  ) {
    return "special";
  }
  if (num.length === 10) {
...
const _pn_getHyphenPattern = (number, type) => {
  ...
  if (
    ...
    if (
      num.length === 11 &&
      _pn_isDigit11PhoneNumberRange(num.substring(0, 4))
    ) {
      return [4, 3, 4];
    }
    return [3, 4, 4];
...
const _pn_format11DigitMobile = (num) => {
  ...
  for (let c = 0, l = areaCodeList.length; c < l; c++) {
    ...
    let localAreaCodeLen = _pn_getAreaCodeInfo(
      areaCodeLen,
      telephoneNumberAreaCode,
    );
    if (
      localAreaCodeLen &&
      _pn_isDigit11PhoneNumberRange(telephoneNumberAreaCode)
    ) {
      const local_code = Number(
        num.substring(areaCodeLen, areaCodeLen + localAreaCodeLen),
      );
...
const _pn_fallbackLandlineFormat = (num) => {
  ...
  for (let c = 0, l = areaCodeList.length; c < l; c++) {
    ...
    if (localAreaCodeLen) {
      let landlineFlag = !_pn_isNotLandlinePhoneNumberRange(
        telephoneNumberAreaCode,
      );
      ...
      if (landlineFlag) {
        ...
        if (!rejectFlag) {
          ...
          if (!range) {
            ...
            rejectFlag = !range.some(
              ([min, max]) => local_code >= min && local_code <= max,
            );
          }
        }
      } else {
        const local_area_code_flag = _pn_getLocalAreaCodeRange(
          telephoneNumberAreaCode,
        );
...
const _pn_isValidJapanesePhoneNumber = (str) => {
  ...
  if (num.length === 10) {
    const prefix4 = num.substring(0, 4);
    const prefix3 = num.substring(0, 3);
    const digit11Prefixes = Object.values(
      _pn_phoneNumberData.digit11PhoneNumberRange,
    ).flat();
    if (
      digit11Prefixes.includes(prefix4) ||
      digit11Prefixes.includes(prefix3)
    ) {
      return false;
    }
...
const phone_number_formatting = (telephoneNumber) => {
  ...
  if (!_pn_isValidJapanesePhoneNumber(telephoneNumber)) {
    throw new Error(
      `無効な日本国内電話番号です（桁数・形式が不正です）: ${telephoneNumber}`,
    );
  }
  // 正規化
  ...
  if (range) {
    const local_code_str = num.substring(
      matchedAreaCodeLen,
      matchedAreaCodeLen + matchedLocalLen,
    );
...
// 公開: 電話番号変換ユーティリティをグローバルに露出（ファイル末尾に追加）
if (typeof window !== "undefined") {
  try {
    window.phone_number_formatting_with_type =
      typeof phone_number_formatting_with_type !== "undefined"
        ? phone_number_formatting_with_type
        : undefined;
  } catch {}
}
