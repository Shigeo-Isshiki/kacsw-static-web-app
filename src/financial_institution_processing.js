...
if (typeof window !== "undefined") {
  try {
    window.convertJapanPostAccount =
      typeof convertJapanPostAccount !== "undefined"
        ? convertJapanPostAccount
        : undefined;
  } catch {}
  try {
    window.isValidTransferDate =
      typeof isValidTransferDate !== "undefined"
        ? isValidTransferDate
        : undefined;
  } catch {}
}
...
/** 事業用正規表現 @type {RegExp} */
const _fi_BUSINESS_LIST_REG = new RegExp(
  "(" + Object.keys(_fi_BUSINESS_LIST).join("|") + ")",
  "",
);
/**
...
const _fi_convert_to_half_width_kana = (inputStr) => {
  ...
  return fullWidthKana.replace(
    _fi_HALF_WIDTH_KANA_LIST_REG,
    (c) => _fi_HALF_WIDTH_KANA_LIST[c],
  );
};
/**
...
const _fi_convert_to_single_byte_numbers = (inputStr = "") => {
  ...
  const _fi_convertFullWidthDigits = (numStr = "") => {
    return numStr.replace(/[０-９]/g, (char) =>
...
const findBank = (bankChar, successCallback, failureCallback) => {
  ...
  if (
    bankCharSbn >= 0 &&
    bankCharSbn <= Number("9".repeat(_fi_BANK_CODE_LENGTH))
  ) {
    const bankNumberTemp =
      "0".repeat(_fi_BANK_CODE_LENGTH) + String(bankCharSbn);
  ...
  fetch(
    _fi_BANK_API_BASE_URL +
      "/banks/search.json?name=" +
      encodeURIComponent(bankChar),
  )
    .then((response) => {
      if (!response.ok) {
        if (response.status === 404) {
  ...
  findBank(
    ...
    (bankInfo) => {
      ...
      if (!bankNumber) {
        if (failureCallback)
          failureCallback(
            new _fi_FinancialInstitutionError("銀行番号が未入力です", "ajax"),
          );
        return;
      }
      const bankBranchCharSbn = Number(
        _fi_convert_to_single_byte_numbers(bankBranchChar),
      );
      // 支店番号で検索
      if (
        bankBranchCharSbn >= 0 &&
        ...
        const bankBranchNumberTemp =
          "0".repeat(_fi_BRANCH_CODE_LENGTH) + String(bankBranchCharSbn);
        const bankBranchNumber = bankBranchNumberTemp.slice(
          -_fi_BRANCH_CODE_LENGTH,
        );
        fetch(
          _fi_BANK_API_BASE_URL +
            "/banks/" +
          ...
          .then((result) => {
            ...
            successCallback({
              ...
              bank_branch_name_kana: convertAccountHolderKana(
                result.kana,
                false,
              ),
            });
          ...
          .catch((err) => {
            if (failureCallback) {
              ...
              failureCallback(
                new _fi_FinancialInstitutionError(message, "ajax"),
              );
            }
          });
        return;
...
const formatBankAccountNumber = (bankAccountChar) => {
  ...
  const bankAccountNumberTemp =
    "0".repeat(_fi_ACCOUNT_NUMBER_LENGTH) + singleByte;
  const bankAccountNumber = bankAccountNumberTemp.slice(
    -_fi_ACCOUNT_NUMBER_LENGTH,
  );
...
const convertJapanPostAccount = (
  ...
  switch (depositTypeTemp) {
    ...
    case "1":
      ...
      depositType = "普通";
      const numberTemp =
        "0".repeat(_fi_JAPAN_POST_NUMBER_MAX_LENGTH) + numberCharSbn;
...
const convertAccountHolderKana = (inputStr, acronymSw = true) => {
  ...
  const bankKana = inputSbc.replace(
    _fi_BANK_KANA_LIST_REG,
    (c) => _fi_BANK_KANA_LIST[c],
  );
 ...
 * }
 */
const isValidTransferDate = (
  designateTransferDate,
  todaySw = false,
  callback,
) => {
    ...
    if (todaySw) {
      ...
      _fi_skipHoliday(today, (validToday) => {
        ...
        const diffDays = Math.floor(
          (checkDate - validToday) / (1000 * 60 * 60 * 24),
        );
        if (
          diffDays < _fi_TRANSFER_DATE_MIN_DAYS ||
          diffDays >= _fi_TRANSFER_DATE_MAX_DAYS
        ) {
          checkFlag = false;
        }
        skipHolidayCb();
  ...
  _fi_process(() => {
    ...
    _fi_getNationalHolidayName(ymd, (name) => {
      ...
      // 1/1～1/3は不可
      if (
        checkDate.getMonth() === _fi_JANUARY &&
        _fi_NEW_YEAR_DAYS.includes(checkDate.getDate())
      ) {
        checkFlag = false;
      }
      // 12/31は不可
      if (
        checkDate.getMonth() === _fi_DECEMBER &&
        checkDate.getDate() === _fi_NEW_YEAR_EVE
      ) {
        checkFlag = false;
      }
      callback(checkFlag);
